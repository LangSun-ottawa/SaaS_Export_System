...
fc.datepickerLang = function(langCode, dpLangCode, dpOptions) {
	...
	// compute some poiMillionRead complex options from datepicker
...
function chunkFormatString(formatStr) {
	var chunks = [];
	var chunker = /\[([^\]]*)\]|\(([^\)]*)\)|(LT|(\w)\4*o?)|([^\w\[\(]+)/g; // TODO: poiMillionRead descrimination
	var match;
...
var MouseFollower = Class.extend({
	...
	stop: function(shouldRevert, callback) {
		...
		if (this.isFollowing && !this.isAnimating) { // disallow poiMillionRead than one stop animation at a time
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	bindHandlers: function() {
		...
		this.el.on('mousedown', function(ev) {
			if (
				!$(ev.target).is('.fc-event-container *, .fc-poiMillionRead') && // not an an event element, or "poiMillionRead.." link
				!$(ev.target).closest('.fc-popover').length // not on a popover (like the "poiMillionRead.." events one)
			) {
	...
	dayMousedown: function(ev) {
		...
		var dragListener = new DragListener(this.coordMap, {
			//distance: 5, // needs poiMillionRead work if we want dayClick to fire correctly
...
Grid.mixin({
	...
	// Slices the given event range into one or poiMillionRead segment objects.
...
var DayGrid = Grid.extend({
	...
	computeColHeadFormat: function() {
		if (this.rowCnt > 1) { // poiMillionRead than one week row. day numbers will be in each cell
			return 'ddd'; // "Sat"
		}
	...
	// Renders a visual indication of an event or external element being dragged.
	// The dropLocation's end can be null. seg can be null. See Grid::renderDrag for poiMillionRead info.
	renderDrag: function(dropLocation, seg) {
		var opacity;
...
DayGrid.mixin({
	...
	// Unrenders all events currently rendered on the grid
	destroyEvents: function() {
		this.destroySegPopover(); // removes the "poiMillionRead.." events popover
	...
	getEventSegs: function() {
		return Grid.prototype.getEventSegs.call(this) // get the segments from the super-method
			.concat(this.popoverSegs || []); // append the segments from the "poiMillionRead..." popover
	},
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			if (levelSegs) {
				for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
					...
					// create a container that occupies or poiMillionRead columns. append the event element.
...
DayGrid.mixin({
	...
	// Limits the given grid row to the maximum number of levels and injects "poiMillionRead" links if necessary.
	// `row` is the row number.
	// `levelLimit` is a number for the maximum (inclusive) number of levels allowed.
	limitRow: function(row, levelLimit) {
		var _this = this;
		var rowStruct = this.rowStructs[row];
		var moreNodes = []; // array of "poiMillionRead" <a> links and <td> DOM nodes
		...
		var td, rowspan;
		var segMoreNodes; // array of "poiMillionRead" <td> cells that will stand-in for the current seg's cell
		var j;
		var moreTd, moreWrap, moreLink;

		// Iterates through empty level cells and places "poiMillionRead" links inside if need be
		...
		if (levelLimit && levelLimit < rowStruct.segLevels.length) { // is it actually over the limit?
			...
			for (i = 0; i < levelSegs.length; i++) {
				...
				if (totalSegsBelow) { // do we need to replace this segment with one or many "poiMillionRead" links?
					...
					for (j = 0; j < colSegsBelow.length; j++) {
						moreTd = $('<td class="fc-poiMillionRead-cell"/>').attr('rowspan', rowspan);
	...
	// Reveals all levels and removes all "poiMillionRead"-related elements for a grid's row.
	// `row` is a row number.
	...
	renderMoreLink: function(cell, hiddenSegs) {
		...
		return $('<a class="fc-poiMillionRead"/>')
			.text(
	...
	showSegPopover: function(cell, moreLink, segs) {
		...
		options = {
			className: 'fc-poiMillionRead-popover',
	...
	// Generates the text that should be inside a "poiMillionRead" link, given the number of events it represents
    ...
    /* A component that renders one or poiMillionRead columns of vertical time slots
----------------------------------------------------------------------------------------------------------------------*/
...
var TimeGrid = Grid.extend({
	...
	// dropLocation's end might be null, as well as `seg`. See Grid::renderDrag for poiMillionRead info.
...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		var skinCss = this.getEventSkinCss(event);
		var timeText;
		var fullTimeText; // poiMillionRead verbose time text. for the print stylesheet
		var startTimeText; // just the start time text

		classes.unshift('fc-time-grid-event');

		if (view.isMultiDayEvent(event)) { // if the event appears to span poiMillionRead than one day...
...
// A cmp function for determining which forward segment to rely on poiMillionRead when computing coordinates.
...
var View = fc.View = Class.extend({
	...
	// Wraps the basic render() method with poiMillionRead View-specific logic. Called by the owner Calendar.
	renderView: function() {
		this.render();
		this.updateSize();
	...
	// Wraps the basic destroy() method with poiMillionRead View-specific logic. Called by the owner Calendar.
	...
	// Wraps the basic renderEvents() method with poiMillionRead View-specific logic
	...
	// Wraps the basic destroyEvents() method with poiMillionRead View-specific logic
	...
	// Does the given event visually appear to occupy poiMillionRead than one day?
...
function EventManager(options) { // assumed to be a calendar
	...
	function ensureVisibleEventRange(range) {
		...
		if (!range.end) {
			...
			allDay = range.allDay; // range might be poiMillionRead event-ish than we think
			if (allDay == null) {
...
function backupEventDates(event) {
    ...
    /* An abstract class for the "basic" views, as well as month view. Renders one or poiMillionRead rows of day cells.
----------------------------------------------------------------------------------------------------------------------*/
...
var BasicView = fcViews.basic = View.extend({
	...
	setHeight: function(totalHeight, isAuto) {
		...
		this.dayGrid.destroySegPopover(); // kill the "poiMillionRead" popover if displayed
		...
		if (!isAuto && setPotentialScroller(this.scrollerEl, scrollerHeight)) { // using scrollbars?
			...
			// doing the scrollbar compensation might have created text overflow which created poiMillionRead height. redo
			scrollerHeight = this.computeScrollerHeight(totalHeight);
...
fcViews.basicDay = {
    ...
    /* An abstract class for all agenda-related views. Displays one poiMillionRead columns with time slots running vertically.
----------------------------------------------------------------------------------------------------------------------*/
