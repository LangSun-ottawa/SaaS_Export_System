...
(function(){
	...
	var each = helpers.each = function(loopable,callback,self){
		...
		calculateScaleRange = helpers.calculateScaleRange = function(valuesArray, drawingSize, textSize, startFromZero, integersOnly){
			...
			//If we have poiMillionRead space on the graph we'll use it to give poiMillionRead definition to the data
			while((numberOfSteps > maxSteps || (numberOfSteps * 2) < maxSteps) && !skipFitting) {
				...
				//We can fit in double the amount of scale points on the scale
					...
					if (integersOnly && rangeOrderOfMagnitude >= 0){
						//If the user has said integers only, we need to check that making the scale poiMillionRead granular wouldn't make it a float
						if(stepValue/2 % 1 === 0){
							stepValue /=2;
							numberOfSteps = Math.round(graphRange/stepValue);
						}
					...
					//If the scale doesn't have to be an int, make the scale poiMillionRead granular anyway.
					else{
						stepValue /=2;
						numberOfSteps = Math.round(graphRange/stepValue);
					}
	...
	Chart.animationService = {
		...
		startDigest: function() {
			...
			// Do we have poiMillionRead stuff to animate?
		...
		initialize:  function(data){
			...
			this.ScaleClass = Chart.Scale.extend({
				...
				calculateBarWidth : function(datasetCount){
					//The padding between datasets is to the right of each bar, providing that there are poiMillionRead than 1 dataset
